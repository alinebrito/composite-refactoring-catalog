...
class RefDiff:
    ...
    def detect_refactorings(self, project, language):
        ...
        path = 'output/{}/results'.format(project)
        ...
        with open('{}/refactorings.csv'.format(path), 'a+') as file:
            ...
            for index, commit in commits.iterrows():
                ...
                proc = subprocess.Popen(['java', '-cp', '"bin/refdiff_lib/*"', '-jar', 'bin/refdiff.jar', 'output', '{}/code'.format(project), language, commit.get('commitHash'), '-Xmx6144m'], stdout=subprocess.PIPE).communicate()
                for data in proc:
                    if data:
                        ...
                        for ref in refactorings:
                            ...
                            os.fsync(file.fileno())
                            file.write('{}\n'.format(line[:-1]))
                subprocess.call('rm -R output/code-{}'.format(commit.get('commitHash')[0:7]), shell = True)
